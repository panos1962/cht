...
CREATE TABLE `paralogos` (
	...
	-- Το πεδίο "logos" που ακολουθεί αποτελεί μέρος του primary key
	-- του πίνακα "paralogos" και αριθμεί τους λόγους παράβασης ανά
	-- είδος παράβασης. Το πεδίο χρησιμοποιείται ως αναφορά και ως
	-- εκ τούτου δεν μπορεί να αλλάξει στο μέλλον. Αν κάποιο είδος
	-- πάψει να είναι σε χρήση, τότε απλώς συμπληρώνουμε το πεδίο
	-- "anenergos" με την ημερομηνία παύσης, ωστόσο οι παραβάσεις
	-- που έχουν βεβαιωθεί με τον συγκεκριμένο αύξοντα αριθμό δεν
	-- αλλάζουν.
