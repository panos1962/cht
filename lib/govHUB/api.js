...
gh.carGet = (input, callback) => {
	...
	if (!input.data.hasOwnProperty('oxima')) {
		output.error = "missing oxima";
		callback(output);
		return;
	}

	try {
		input.data.oxima = data.oxima.toString().toUpperCase();
	}

	catch (e) {
		output.error = "invalid oxima";
		callback(output);
		return;
	}

	if (!input.data.hasOwnProperty('date'))
	input.data.date = (new Date()).toISOString().substring(0, 10);

	else if ((typeof(data.date) === 'object') &&
		(data.date instanceof Date))
	input.data.date = data.date.toISOString().substring(0, 10);
	...
	gh.txId++;
	pd.ttymsg('Requesting data for "' + input.data.oxima + '"...');
	request.post({
		...
		'body': JSON.stringify({
			...
			'getVehicleInformationInputRecord': {
				'arithmosKykloforias': input.data.oxima,
				'requestDate': input.data.date,
			},
		}),
	}, (err, rsp, body) => {
		pd.ttymsg().errchk(err);
		...
		try {
			...
			output.error = 'invalid returned data';
			callback(output);
			return;
		}

		if (!body.hasOwnProperty('getVehicleInformationOutputRecord')) {
			output.error = 'δεν επεστράφησαν στοιχεία οχήματος';
			callback(output);
			return;
		}

		let oxima = data.getVehicleInformationOutputRecord;

		if (oxima.arithmosKykloforias !== input.data.oxima) {
			output.error = 'επεστράφη διαφορετικός αρ. κυκλοφορίας';
			callback(output);
			return;
		}

		output.data = (new gh.oxima()).fromGovHUB(vehicle);
		callback(output);
	});
};
