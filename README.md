...
Η διαδικασία που περιγράψαμε παραπάνω μπορεί να γίνει ηλεκτρονικά μέσω ειδικής διαδικτυακής εφαρμογής που έχει αναπτύξει το Τμήμα Μηχανογραφικής Υποστήριξης του Δήμου Θεσσαλονίκης. Πράγματι, εφόσον ο αρμόδιος υπάλληλος φέρει μαζί του ηλεκτρονική συσκευή με σύνδεση στο Internet (laptop, PDA, tablet, κινητό τηλέφωνο κλπ), μπορεί με πολύ απλό τρόπο και με μεγάλη ταχύτητα να καταγράφει τις παραβάσεις τις οποίες διαπιστώνει κατά τη διαδρομή που ακολουθεί στο πεδίο. Επιπλέον, η ηλεκτρονική καταγραφή των παραβάσεων ΚΟΚ διασφαλίζει σε πολύ μεγάλο βαθμό την αποφυγή λαθών, ενώ η καθαρότητα της καταγραφής σε καμία περίπτωση δεν συγκρίνεται με την χειρόγραφη καταγραφή ακόμη και αν ο υπάλληλος είναι καλλιγράφος.

Όμως, τα μεγαλύτερα οφέλη της ηλεκτρονικής καταγραφής παραβάσεων ΚΟΚ δεν είναι ούτε η ταχύτητα ούτε η καθαρότητα της καταγραφής· τα μεγαλύτερα οφέλη της ηλεκτρονικής καταγραφής παραβάσεων ΚΟΚ είναι:
...
Βεβαίως, με την ηλεκτρονική καταγραφή παραβάσεων ΚΟΚ ανακύπτουν κάποια θέματα νομικής φύσης που αφορούν κυρίως στην ενημέρωση του οδηγού σχετικά με την παράβαση στην οποία έχει υποοπέσει. Πράγματι, όπως έχουμε ήδη αναφέρει, ο υπάλληλος που βεβαιώνει την παράβαση και επιβάλλει το πρόστιμο και τυχόν άλλες κυρώσεις, πρέπει να εναποθέτει σε εμφανές σημείο του οχήματος το φύλλο στο οποίο έχει καταγράψει τα στοιχεία της παράβασης. Κατά την ηλεκτρονική καταγραφή δεν υφίσταται καν το προβλεπόμενο από το νόμο καρνέ καταγραφής, επομένως δεν υπάρχει δυνατότητα αποκοπής φύλλων και εναπόθεσης κλήσεων σε εμφανές σημείο των οχημάτων. Αντ' αυτού θα πρέπει ή να εκτυπώνεται η κλήση από φορητό εκτυπωτή που θα πρέπει να φέρει μαζί του ο αρμόδιος υπάλληλος, ή πιο απλά να εναποτίθεται σε εμφανές σημείο των οχημάτων προτυπωμένο φύλλο με τον αύξοντα αριθμό της παράβασης και σχετικές οδηγίες με τις οποίες ο κάτοχος του οχήματος θα μπορεί να ενημερωθεί σχετικά.

Η δεύτερη λύση προκρίνεται καθώς αφενός αποφεύγουμε τη χρήση φορητών εκτυπωτών και ό,τι αυτό συνεπάγεται (αγορά και συντήρηση συσκευών και αναλωσίμων, μικρότερο βάρος εξοπλισμού υπαλλήλων κλπ) και αφετέρου μειώνουμε στο ελάχιστο τη δημοσιοποίηση δεδομένων που αφορούν στην εν λόγω παράβαση, καθώς στο προτυπωμένο έντυπο δεν θα υπάρχει απολύτως κανένα στοιχείο σχετικό με την όποια παράβαση, παρά μόνον ο αριθμός της παράβασης τον οποίο θα μπορεί να χρησιμοποιήσει μόνον όποιος γνωρίζει το ΑΦΜ του κύριου κατόχου του εμπλεκομένου οχήματος, προκειμένου να επισκεφθεί τη σχετική ενημερωτική σελίδα στην οποία αναρτώνται όλα τα στοιχεία της συγκεκριμένης παράβασης.
