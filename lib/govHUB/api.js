...
// "govHUB" του Υπουργείου Εσωτερικών, η οποία στο παρόν θα αναφέρεται απλώς
// ως «πλατφόρμα».
//
// Updated: 2019-12-19
// Updated: 2019-12-18
...
// και θέτει αναλόγως τα πεδία τού ανά χείρας "prosopo" object πραλλάσσοντας
...
gh.prosopo.prototype.onomasiaFix = function() {
	let onomasia = this.onomasia;
	delete this.onomasia;

	let a = onomasia.split(',');
	let eponimo1 = '';
	let eponimo2 = '';
	this.onoma = '';
	this.patronimo = '';
...
// property "client.accessToken". Αν δεν καθορίσουμε callback function, τότε
...
gh.tokenGet = (conf, client, callback) => {
	...
	gh.monitor('Requesting connect token\n');
	request.post({
		...
		if (!callback)
		return console.log(client.accessToken.access_token);
