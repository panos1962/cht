...
Dimas.paravidos.misthosiKatigoria = [
	'ΜΙΣΘΩΜΕΝΟ',
	'ΤΑΞΙ',
	'ΑΓΟΡΑΙΟ',
	'ΔΗΜΟΣΙΑΣ ΧΡΗΣΗΣ',
...
Dimas.paravidos.dikikloKatigoria = [
	'ΔΙΚΥΚΛΟ',
];
...
Dimas.paravidos.oximaKatigoria = [];
Dimas.paravidos.oximaKatigoriaList = {};

pd.arrayWalk(Dimas.paravidos.misthosiKatigoria, (x, i) => {
	Dimas.paravidos.misthosiList[x] = i;
	Dimas.paravidos.oximaKatigoria.push(x);
	Dimas.paravidos.oximaKatigoriaList[x] = i;
});

pd.arrayWalk(Dimas.paravidos.dikikloKatigoria, (x, i) => {
	Dimas.paravidos.dikikloList[x] = i;
	Dimas.paravidos.oximaKatigoria.push(x);
	Dimas.paravidos.oximaKatigoriaList[x] = i;
});

Dimas.paravidos.prototype.kirosiGet = function(kirosi, katigoria) {
	if (katigoria === undefined)
	return this[kirosi];

	if (Dimas.paravidos.misthosiList.hasOwnProperty(katigoria)) {
		switch (kirosi) {
		case 'prostimo':
	...
	if (Dimas.paravidos.dikikloList.hasOwnProperty(katigoria)) {
		switch (kirosi) {
		case 'prostimo':
...
Dimas.paralogos = function(obj) {
	pd.objectInit(this, obj);
};

Dimas.paralogos.fromParalogosList = function(s) {
	return eval('new Dimas.paralogos(' + s + ')');
};
