...
// Παρακάτω ορίζουμε λίστα με τα properties που είναι αναμενόμενα από την
// ΓΓΠΣ. Σε περίπτωση που παραλάβουμε property που δεν περιλαμβάνεται στη
// λίστα που ακολουθεί θα εκτυπώσουμε warning message προκειμένου να ελεγχθεί
// η νέα property και τι αυτή σημαίνει. Σε κάθε αναμενόμενη property δίνουμε
// τιμή true εφόσον η συγκεκριμένη property μας ενδιαφέρει, αλλιώς δίνουμε
// τιμή false.

gh.prosopo.platformValidProperties = {
...
// και θέτει αναλόγως τα πεδία τού ανά χείρας "prosopo" object.

gh.prosopo.prototype.fromGsis = function(rsp, gsisData) {
	return gh.fromPlatformData(rsp, 'prosopo', this, gsisData, {
		"afm": "afm",
		"doy": "doi",
		"doyDescr": "doiDesc",
		"appelation": "eponimia",
...
gh.katoxos.prototype.fromGovHUB = function(rsp, govHUBData) {
	return gh.fromPlatformData(rsp, 'katoxos', this, govHUBData, {
		"afm": "afm",
		"percent": "pososto",
		"doy": "doi",
		"doyDesc": "doiDesc",
...
gh.oxima.platformValidProperties = {
...
gh.oxima.prototype.fromGovHUB = function(rsp, govHUBData) {
	gh.fromPlatformData(rsp, 'oxima', this, govHUBData, {
...
gh.tokenGet = (conf, client, callback) => {
...
gh.afmGet = (opts, callback) => {
	...
	pd.ttymsg('Requesting data for "' + keyData.afm + '"...\n');
	request.post({
		'url': conf.prosopoInfo.url,
		'headers': {
			'Authorization': 'Bearer ' + conf.connect.accessToken,
			'Content-Type': 'application/json',
		},
...
gh.responseBodyCheck = (opts, objName, body, callback) => {
	...
	for (let i in body) {
		if (gh[objName].platformValidProperties.hasOwnProperty(i))
		continue;
...
gh.responseErrorCheck = (opts, objName, body, callback) => {
...
// Η function "fromPlatformData" δέχεται ως παραμέτρους ένα target object, ένα
...
gh.fromPlatformData = function(rsp, objClass, target, platformData, propMap) {
